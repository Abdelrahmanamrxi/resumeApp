...
<motion.div
  ...
  className="w-full md:w-3/5 bg-white rounded-lg shadow-lg p-6 max-h-[calc(100vh-2rem)] overflow-y-auto border-l-4"
>
  <motion.header
    initial={{ y: -20, opacity: 0 }}
    animate={{ y: 0, opacity: 1 }}
    transition={{ delay: 0.2 }}
    style={{ backgroundColor: getColor(resumeData.color.accentColor, 600) }}
  ...
  >
    <h2
      style={{ backgroundColor: getColor(resumeData.color.accentColor, 600) }}
      className={`text-sm font-bold ${resumeData.summary ? "text-white px-2 py-1 rounded mb-2" : ""}`}
    >
      {resumeData.summary ? "PROFESSIONAL SUMMARY" : ""}
    </h2>
    <p 
      style={{ color: getColor(resumeData.color.textColor, 600) }}
      className="text-xs leading-relaxed bg-gray-50 p-3 rounded"
    >
      {resumeData.summary}
    </p>
  </motion.section>
  ...
  >
    <h2
      style={{ backgroundColor: getColor(resumeData.color.accentColor, 600) }}
      className={`text-sm font-bold ${resumeData.experiences.length > 0 ? "text-white px-2 py-1 rounded mb-2" : ""}`}
    >
    ...
    <AnimatePresence>
      {resumeData.experiences.map((exp, i) => (
        ...
        >
          ...
          <p  className='text-sm text-gray-500'>{exp.from.toLocaleDateString("en-US",{month:'short',year:"numeric"})} - {exp.To.toLocaleDateString("en-US",{month:'short',year:"numeric"})}</p>
            </div>
          <div 
  ...
  >
    <h2
      style={{ backgroundColor: getColor(resumeData.color.accentColor, 600) }}
      className={`text-sm font-bold ${resumeData.education.length > 0 ? "text-white px-2 py-1 rounded mb-2" : ""}`}
    >
    ...
    <div className="bg-gray-50 p-3 rounded">
      {resumeData.education.map((edu, i) => (
        ...
        <motion.p 
          ...
          transition={{ delay: i * 0.1 }}
          >
          <strong style={{ color: getColor(resumeData.color.textColor, 800) }}>
            <strong>{edu.degree}</strong>, {edu.institution}
          </strong>
        </motion.p>
        <p className='text-sm text-gray-500'>
          {edu.from.toLocaleDateString("en-US",{month:'short',year:"numeric"})} - {edu.To.toLocaleDateString("en-US",{month:'short',year:"numeric"})}
          </p> 
      </div> 
      ))}
    </div>
  ...
  {resumeData.certifications.length > 0 && (
    ...
    >
      <h2
        style={{ backgroundColor: getColor(resumeData.color.accentColor, 600) }}
        className="text-sm font-bold text-white px-2 py-1 rounded mb-2"
      >
        CERTIFICATIONS
      </h2>
      <div className="bg-gray-50 p-3 rounded">
        {resumeData.certifications.map((cert, i) => (
          <div className='flex-row items-center justify-between flex mt-2'>
          <motion.p 
            key={i} 
            style={{ color: getColor(resumeData.color.textColor, 600) }}
            className="text-xs mb-1"
          ...
          <p className='text-gray-500 text-sm'>{new Date(cert.year).toLocaleDateString("en-US",{month:'short',year:"numeric"})}</p>
            </div> 
        ))}
      </div>
    </motion.section>
  ...
  >
    <h2
      style={{ backgroundColor: getColor(resumeData.color.accentColor, 600) }}
      className={`text-sm font-bold ${(resumeData.skills.technical.length > 0 || resumeData.skills.soft.length > 0) ? "text-white px-2 py-1 rounded mb-2" : ""}`}
    >
